...
export default function TaskPage() {
  ...
  const task = useMemo(() => {
    const builtIn = tasks.find((t) => t.id === taskId);
    if (builtIn) return builtIn;
    return getCustomTasks().find((t) => t.id === taskId) ?? null;
  }, [taskId]);
  const localTitle = (task: { title: string; titleEn?: string }) =>
  ...
  const rawChecklist = useMemo(() => checklists[taskId] ?? getCustomChecklist(taskId), [taskId]);
  ...
  const generatePdf = useCallback(async () => {
    ...
    try {
      ...
      doc.text("OK Sécurité", margin, 18);
      doc.setFontSize(11);
      doc.setFont("helvetica", "normal");
      doc.text(localTitle(task), margin, 28);
      doc.setFontSize(9);
      ...
      <main className="flex-1 px-5 py-4 sm:px-8" aria-label={t("a11y.checklist")}>
        {taskId === "travaux-hauteur" && (
          <div className="mb-4 overflow-hidden rounded-xl">
            <iframe
              src="https://www.youtube.com/embed/P0MeUIPE4Ns"
              title="Video"
        ...
        <div className="fixed inset-0 z-[100] flex flex-col bg-black" role="dialog" aria-label={t("task.scanTitle")}>
            ...
            <div className={`relative w-full overflow-hidden ${scanState === "done" ? "flex-shrink-0" : "flex-1"}`} style={scanState === "done" ? { height: "42dvh" } : undefined}>
              ...
              {scanState === "scanning" && (
                <>
                  ...
                  <div className="animate-scan-line absolute inset-x-0 h-[2px]" style={{ filter: "drop-shadow(0 0 12px rgba(34,197,94,0.9))" }}>
          ...
          {scanState === "done" && (
            <div className="flex-1 overflow-y-auto px-5 pb-[calc(env(safe-area-inset-bottom)+1.5rem)] pt-5">
              ...
              <button
                onClick={closeScan}
                className="mt-5 w-full rounded-xl bg-primary py-3.5 font-heading text-sm font-bold text-white transition-colors active:bg-primary-dark"
              >
                {t("task.scanClose")}
              </button>
            </div>
